...
/** The vhost's `location` blocks as { spec, body }, braces counted so an inner `if` stays inside. */
function locationBlocks(): { spec: string; body: string }[] {
  const text = fs.readFileSync(VHOST, 'utf8')
  const blocks: { spec: string; body: string }[] = []
  for (const m of text.matchAll(/^\s*location\s+([^{]+?)\s*\{/gm)) {
    ...
    blocks.push({ spec: m[1], body: text.slice(from, i - 1) })
  }
  return blocks
}
...
/**
 * Every location that forwards to Node is behind an edge limit.
 *
 * `location = /bridge` had none at all: an exact match beats the `/` prefix, so
 * a single address could pour ordinary HTTP requests straight through nginx at
 * ~44x what oc_general allows it on every other path, and the zones file's
 * "a flood never reaches Node in the first place" was false for that path.
 * The upgrade guard is the other half: the bridge's socket is the only thing
 * this location exists for, so anything that is not a WebSocket handshake is
 * answered by nginx and never proxied at all.
 */
test('every proxying location carries an edge limit, and /bridge proxies only a WebSocket upgrade', () => {
  const proxying = locationBlocks().filter((l) => /^\s*proxy_pass\s/m.test(l.body))
  expect(proxying.map((l) => l.spec)).toContain('= /bridge')

  for (const loc of proxying) {
    expect(loc.body, `location ${loc.spec} has no limit_req`).toMatch(/^\s*limit_req\s+zone=/m)
    // limit_conn only where a connection can stay open: the API endpoints are
    // request/response and their limit_req already bounds what is in flight,
    // but a viewer's SSE and the bridge's socket live for hours.
    if (loc.spec === '/' || loc.spec === '= /bridge') {
      expect(loc.body, `location ${loc.spec} has no limit_conn`).toMatch(/^\s*limit_conn\s+/m)
    }
  }

  const bridge = proxying.find((l) => l.spec === '= /bridge')!
  expect(bridge.body, 'no $http_upgrade guard on = /bridge').toMatch(/if\s*\(\s*\$http_upgrade\s*!~\*?\s*\^?websocket/i)
  expect(bridge.body, 'the guard must answer, not proxy').toMatch(/return\s+404/)
})

test("DEPLOY.md quotes nginx's edge limits as the repo's nginx config sets them", () => {
  const expected = nginxLimits()
  expect([...expected.keys()].sort()).toEqual(['oc_activate', 'oc_bridge', 'oc_general', 'oc_register'])
...
test("config's list of tracking maps names exactly the Store's bounded maps", () => {
  ...
  // Two ways to be bounded by the cap. Most maps hand an insert to setBounded /
  // addBounded, which evicts the oldest entry. The claims map keeps the cap but
  // not that policy — evicting the oldest reservation is how a registration
  // flood used to take an id from its owner (see Store.makeClaimRoom) — so it
  // checks the cap itself, and is counted here for doing so.
  const bounded = new Set([
    ...[...store.matchAll(/this\.(?:setBounded|addBounded)\(this\.(\w+),/g)].map((m) => m[1]),
    ...[...store.matchAll(/this\.(\w+)\.size >= this\.maxTrackingEntries/g)].map((m) => m[1]),
  ])
