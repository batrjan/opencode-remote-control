...
test('reloading a subagent page, or opening it in a new tab, serves the viewer the UI', async () => {
  ...
  // Proving a subagent takes one parent walk PER SIDE, made once: the relay
  // walks to decide the page, and the bridge walks again to authorize the
  // relay's own probe (it no longer takes the relay's word for which session a
  // path may address — see matchProxyRoute in bridge/src/relay.ts). Both sides
  // cache what they proved, so the other spelling of each page found it known,
  // as the page's own reads of it do next.
  expect(detailReads).toEqual([CHILD, CHILD, GRAND, GRAND])
})
...
test("what the subagent page then reads is the subagent's own, and its parent walk ends at the share", async () => {
  ...
  // A page naming an unrelated session is refused with the marker that sends
  // the tab to its own share's code page. It used to be answered with the
  // share's own detail under the stranger's URL, which is how a tab left open
  // on another share went on working against this one (see
  // cross-share-rebind.test.ts).
  const stranger = await request(relay).get(`/session/${STRANGER}`).set('Cookie', cookie())
  expect(stranger.status).toBe(401)
  expect(stranger.headers['x-oc-relay-auth']).toBe('viewer-invalid')
  expect(stranger.body.id).toBeUndefined()
})
