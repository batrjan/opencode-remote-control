...
async function ownerSharesAgain(X: string, Y: string) {
  ...
  // Y is not this registration's subagent, so nothing of Y is served — and the
  // request is refused rather than quietly answered from X (which is what let a
  // tab whose cookie named another share go on working against this one).
  ...
  expect(sessionDetail.status).toBe(401)
  expect(sessionDetail.body.id).toBeUndefined()
