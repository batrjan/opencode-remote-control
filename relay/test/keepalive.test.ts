...
test('a bridge that stops answering pings is dropped, not left half-open', async () => {
  ...
  // The session's OWN id: a path naming anything else is now refused before the
  // bridge is consulted at all (see cross-share-rebind.test.ts), which would
  // hide the 502 this test is about. It only ever passed on 'sess-silent'
  // because such an id used to be rewritten to the viewer's session.
  const res = await request(relay).get('/session/ses_silent/message').set('x-viewer-token', viewerToken)
...
test('an empty or garbage keep-alive env var falls back to the default, never 0', async () => {
  ...
  // The fourth entry, where a budget has a floor, is a value above it: the
  // probe below asks whether a set value is honoured, not whether the floor is.
  for (const [key, fn, def, set = 1234] of [
