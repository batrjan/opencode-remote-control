  ...
  opencode = createServer((req, res) => {
    ...
    if (req.method === 'GET' && url.pathname === '/event') {
      ...
      const timer = setInterval(() => {
        // A REAL opencode kind: `file.watcher.updated` is on the viewer
        // filter's allow-list of unsessioned kinds (see GLOBAL_EVENT_KINDS),
        // so what this pushes is what a viewer's browser would receive. The
        // made-up `heartbeat` that stood here is not an opencode kind at all
        // and is now dropped, which would have made every test below pass on
        // the relay's own keep-alive instead of on the fan-out.
        res.write(`data: ${JSON.stringify({ type: 'file.watcher.updated', properties: { file: '/path/x.ts', event: 'change' }, n: n++ })}\n\n`)
      }, 50)
...
test('viewer SSE stream receives opencode events pushed via the bridge', async () => {
  ...
  while (!received.includes('"file.watcher.updated"') && Date.now() < deadline) {
    const { value, done } = await reader.read()
    if (done) break
    received += decoder.decode(value, { stream: true })
  }
  await reader.cancel()
  expect(received).toContain('"file.watcher.updated"')
})
...
test('/event forwards the bare event envelope', async () => {
  ...
  expect(forwarded.type).toBe('file.watcher.updated')
})
...
test('/global/event wraps every frame in { directory, payload }', async () => {
  ...
  expect(frames[1]!.payload).toMatchObject({ type: 'file.watcher.updated' })
})
