...
test("another session's permission stays refused, whichever session the path names", async () => {
  for (const sessionID of [CHILD, STRANGER, PARENT]) {
    const res = await request(relay)
      ...
      .set('x-viewer-token', viewerToken)
      .send({ response: 'once' })
    // Two refusals, in front of each other: the relay turns away a session that
    // is not the share's before it routes anything (401, with the marker that
    // sends a mis-cookied tab to its own code page — it used to answer such a
    // path from the bound session instead), and the bridge turns away a
    // permission id that is not the named session's (403).
    expect(res.status).toBe(sessionID === STRANGER ? 401 : 403)
  }
...
test("a subagent's detail is its own, parent link included; a stranger's is refused", async () => {
  ...
  // Refused rather than answered with the share's own detail under the
  // stranger's id: a tab whose cookie now names another share must find out,
  // not carry on (see cross-share-rebind.test.ts).
