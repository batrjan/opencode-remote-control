...
// The shells used to be listed here under their file names, /index.html and
// /join.html, which is how the static handler served them: raw, with no CSP and
// none of the injected scripts. They are served only by their routes now (see
// shell-csp.test.ts), and it is those that must keep revalidating.
test('unhashed fonts and root files keep revalidating (never immutable)', async () => {
  const app = createApp(new Store())
  for (const url of [`/assets/${FIXTURES.inter}`, `/assets/${FIXTURES.mono}`, '/join', '/terminal']) {
