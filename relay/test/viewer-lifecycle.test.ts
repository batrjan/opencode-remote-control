 ...
 * they last used it.
 *
 * The loop is bounded by config.viewerMaxLifetimeMs now, the absolute ceiling
 * over the sliding window. It used to run a simulated month — the cookie's own
 * Max-Age — because a token in use never expired at all. What it asserts is
 * unchanged and holds at every step: each response renews the cookie, for no
 * less than the token's idle window. The tail pins where access now ends.
 */
test('a viewer who keeps using the share keeps their cookie past the lifetime it was issued with', async () => {
  ...
  // The cookie is issued for far longer than a token can now live, so the
  // sliding window is the only thing the visits below can run out of.
  expect(jar.expires!).toBeGreaterThan(joinedAt + config.viewerMaxLifetimeMs)
  ...
  // One visit every 23 h, always inside the token's idle window, for as long as
  // a token may live at all.
  while (Date.now() + config.viewerIdleTtlMs - 3_600_000 < joinedAt + config.viewerMaxLifetimeMs) {
    vi.setSystemTime(Date.now() + config.viewerIdleTtlMs - 3_600_000)
    const at = `${Math.round((Date.now() - joinedAt) / 3_600_000)} h after the join`
  ...
  const token = decodeURIComponent(jar.header()!.slice('viewer_token='.length))
  expect(store.verifyViewer('ses_renew', token)).toBe(true)

  // And the ceiling is real: one more visit, past the absolute lifetime, and
  // the same cookie the relay kept renewing no longer authenticates. The viewer
  // is sent to the code-entry page, the same answer a lapsed token gets.
  vi.setSystemTime(joinedAt + config.viewerMaxLifetimeMs + 1_000)
  const after = await request(app).get(shell).set('Cookie', jar.header()!)
  expect(after.status, 'a token past its absolute lifetime still opened the share').toBe(302)
  expect(after.headers.location).toBe('/ses_renew')
  expect(store.verifyViewer('ses_renew', token)).toBe(false)
})
