...
test('proxy refuses a session id that is not the viewer’s, rather than binding it', async () => {
  // The id used to be rewritten to the viewer's own session without a word,
  // which sent the requests of a tab still open on another share into this one
  // (see cross-share-rebind.test.ts). Now the tab is told, with the marker its
  // shell acts on, and nothing is read for it.
  lastMessagePath = ''
  const res = await request(relay)
...
test(
  ...
  async () => {
    ...
    // Shaped like the real thing: a session.* event names its session under
    // properties.info.id, and one that names none is dropped by the filter
    // (see GLOBAL_EVENT_KINDS in the adapter).
    pushOpencodeEvent(JSON.stringify({ type: 'session.updated', properties: { info: { id: 'ses_sess1' } }, n: 1 }))
    ...
    expect(received).toContain('data: {"type":"session.updated","properties":{"info":{"id":"ses_sess1"}},"n":1}')
  },
  15_000,
)
...
test(
  ...
  async () => {
    ...
    expect(res.status).toBe(200)
    expect(res.headers.get('content-type')).toContain('text/event-stream')
    pushOpencodeEvent(JSON.stringify({ type: 'message.part.updated', properties: { part: { sessionID: 'ses_sess1' } }, n: 2 }))
    ...
    expect(received).toContain(
      'data: {"directory":"/path","payload":{"type":"message.part.updated","properties":{"part":{"sessionID":"ses_sess1"}},"n":2}}',
    )
  },
  15_000,
)
