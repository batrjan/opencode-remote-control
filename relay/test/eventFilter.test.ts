...
test('keeps the allow-listed global events with no session reference', () => {
...
/**
 * Instance-wide events used to pass on the strength of naming no session at
 * all. They no longer do: the stream is scoped to the shared DIRECTORY and
 * not to the session, so "no session id" means "could be anything the owner is
 * doing in that folder" — see GLOBAL_EVENT_KINDS in the adapter and
 * event-global-allowlist.test.ts. Only the kinds on that list still pass.
 */
test('keeps only the allow-listed instance-wide events that mention no session', () => {
  for (const ev of [
  ...
  for (const ev of [
    ...
    expect(eventBelongsToSession(JSON.stringify(ev), 'ses_mine')).toBe(false)
  }
})
