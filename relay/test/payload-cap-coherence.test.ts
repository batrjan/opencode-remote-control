...
test('the inbound budget separates the shares, at the shipped defaults and at any body limit', () => {
  for (const key of ENV) if (key !== 'ACTIVATE_FAIL_DELAY_MS') delete process.env[key]

  // The three numbers are one system, and the arithmetic between them is what
  // the cross-tenant property rests on: the slice is what ONE share may hold,
  // the ceiling what the process may hold, and the reserve what a share holding
  // nothing keeps for a small body however full the relay is. The first shape
  // of them had the slice at exactly one whole body and the ceiling at eight of
  // those, so eight free registrations refused every other share's prompt.
  for (const body of [undefined, String(4 * MiB), String(32 * MiB)]) {
    if (body === undefined) delete process.env.RELAY_PROXY_BODY_LIMIT_BYTES
    else process.env.RELAY_PROXY_BODY_LIMIT_BYTES = body
    const label = `body limit ${proxyBodyLimitBytes() / MiB} MiB`
    // A share can paste twice: never one whole body, whatever the limit is.
    expect(proxyInboundShareBytes(), label).toBeGreaterThanOrEqual(2 * proxyBodyLimitBytes())
    // And eight shares at their slice are not the whole relay.
    expect(proxyMaxInboundBytes(), label).toBeGreaterThanOrEqual(16 * proxyBodyLimitBytes())
    expect(Math.floor(proxyMaxInboundBytes() / proxyInboundShareBytes()), label).toBeGreaterThanOrEqual(8)
    // The reserve is a slice-sized bite of the ceiling at most, and holds room
    // for many small bodies: RESERVE / SMALL is how many quiet shares can be
    // acting through it at once.
    expect(proxyInboundReserveBytes(), label).toBeGreaterThan(0)
    expect(proxyInboundReserveBytes(), label).toBeLessThanOrEqual(proxyInboundShareBytes())
    expect(Math.floor(proxyInboundReserveBytes() / proxyInboundSmallBodyBytes()), label).toBeGreaterThanOrEqual(16)
    // A small body is well under the per-request limit — a prompt, an abort, an
    // answer to a permission — not a paste.
    expect(proxyInboundSmallBodyBytes(), label).toBeLessThan(proxyBodyLimitBytes())
  }

  // A ceiling the operator set above the floor is left exactly as it is, and
  // the slice follows it up.
  delete process.env.RELAY_PROXY_BODY_LIMIT_BYTES
  process.env.RELAY_PROXY_MAX_INBOUND_BYTES = String(1024 * MiB)
  expect(proxyMaxInboundBytes()).toBe(1024 * MiB)
  expect(proxyInboundShareBytes()).toBe(128 * MiB)
})
