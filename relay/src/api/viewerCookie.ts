...
/**
 * The attributes every viewer cookie carries, set and cleared alike.
 *
 * A browser matches a clearing Set-Cookie to the cookie it holds by name, path
 * and domain: get the path wrong and the old cookie simply stays. Shared here
 * so the two can never drift — path is explicit precisely because the token is
 * used on /session/* and /api/* alike and must not inherit the path of whatever
 * route happens to send it.
 */
const VIEWER_COOKIE_ATTRS = {
  httpOnly: true,
  secure: true,
  sameSite: 'strict',
  path: '/',
...
/**
 * Take the viewer cookie back: what POST /api/leave sends beside dropping the
 * token at the relay (see Store.revokeViewer).
 *
 * Both halves matter. Without the revocation the browser would forget a token
 * that still authenticates anyone who kept a copy; without this the browser
 * would keep sending a dead token and land on the code-entry page by way of a
 * 401 on every request instead of going straight there.
 */
export function clearViewerCookie(res: Response): void {
  if (res.headersSent) return
  res.clearCookie('viewer_token', VIEWER_COOKIE_ATTRS)
}
