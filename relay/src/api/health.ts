 ...
 * errors the process survived (see faults.ts), and `claims`, which says whether
 ...
 * not). Neither is public: `faults` moves when a handler throws, so an
...
export const healthRouter = (store: Store) => {
  ...
  r.get('/', (req, res) => {
    ...
    if (fromOperatorSide(req)) return res.json({ ...body, faults: faults(), claims: store.claimStats() })
    res.json(body)
  })
  return r
}
