...
import { proxyAdapter, VIEWER_AUTH_HEADER, VIEWER_AUTH_INVALID, VIEWER_SHARE_HEADER } from './proxy/adapter.js'
import { config, httpRequestTimeoutMs, stateFile, trustProxy, shellCspEnabled } from './config.js'
...
/**
 * Which files in PUBLIC_DIR the static handler is passed over for, leaving the
 * request to the routes behind it and, failing those, to the relay's JSON 404.
 *
 * SOURCE MAPS. The upstream Vite build leaves a .js.map beside every bundle in
 * /assets, 835 of them and about 48 MB (the largest 11.5 MB), and
 * express.static handed each one to anyone who asked, no cookie needed: an
 * anonymous client could make the host send, and nginx gzip, 11.5 MB per
 * request. No viewer needs them. The UI never fetches one, and a browser asks
 * only with DevTools open. They hold the public upstream source, so this saves
 * weight and bandwidth; it hides no secret. The bundles keep their
 * sourceMappingURL comments; a developer with DevTools open sees a 404 for each.
 *
 * HTML DOCUMENTS. PUBLIC_DIR holds the UI shell (index.html) and the join page
 * (join.html), and the relay serves both itself — through sendShell, the ONE
 * place that attaches the shell CSP and injects SERVER_URL_RESET, draftsReset
 * and authGuard. Served raw from disk, the very same SPA came back with none of
 * that: no CSP even with RELAY_SHELL_CSP on, so turning the flag on would have
 * covered less than it claimed, and no authGuard to send a viewer whose cookie
 * is gone or revoked to their code-entry page. A route for the exact
 * '/index.html' is not enough — the file answers to /%69ndex.html as well, and
 * to '//' as a directory index (hence index:false where the handler is built).
 *
 * _headers. The upstream build's deploy-time header config for a static host
 * (Cloudflare Pages and the like). Nothing in a browser reads it, and this
 * relay sets its own headers; served, it is only a description of the build for
 * whoever asks.
 *
 * Checked on the decoded path, as the static handler decodes it before looking
 * on disk: /assets/x.js%2Emap, /%61ssets/x.js.map and /%69ndex.html all name
 * the files their plain spellings do. Case-insensitive, for a filesystem that
 * is (a macOS checkout). A path that does not decode is left to the handler,
 * which refuses it with 400.
 */
function isStaticPassedOver(urlPath: string): boolean {
  let decoded: string
  try {
  ...
  return /\.(?:map|html?)$/i.test(decoded) || /(?:^|\/)_headers$/i.test(decoded)
}
...
/**
 * Guard injected into the UI shell: name the share this page belongs to on
 * every request it makes, and send a viewer whose access ended back to their
 * share's page.
 *
 ...
 *
 * The same wrapper is where the page names its share. requireViewer refuses a
 * request naming a share other than the token's (VIEWER_SHARE_HEADER), which is
 * the only thing between a tab of share B and share A's share-less routes once a
 * join in another tab has replaced the one cookie this origin has — and until
 * now nothing sent that header, so the check could never fire. It can only
 * refuse, never widen: a request carrying no header (a non-browser client, an
 * older shell, /terminal's share-less page) is treated exactly as before.
 */
...
;(() => {
  try {
    ...
    const share = ${JSON.stringify(shareId !== undefined && SHARE_PAGE_ID_RE.test(shareId) ? shareId : '')}
    ...
    window.fetch = function (...args) {
      // Say which share this page belongs to, so the relay can refuse a request
      // driven by another share's token instead of answering it (see
      // VIEWER_SHARE_HEADER). The cookie is one per origin and every share is
      // served from this one, so joining share A in another tab replaces the
      // token this tab authenticates with — and the routes that name no session
      // of their own (/permission, /config, /event) were then answered from A.
      // Same-origin only: a custom header on a cross-origin request would force
      // a CORS preflight the UI never asked for. Everything in try/catch, and
      // the request is passed through untouched if anything here fails — a
      // guard that breaks the page is worse than the mix-up it prevents.
      if (share) {
        try {
          const request = new Request(...args)
          if (new URL(request.url).origin === location.origin) {
            request.headers.set(${JSON.stringify(VIEWER_SHARE_HEADER)}, share)
            args = [request]
          }
        } catch {}
      }
...
const SHELL_CSP_TAIL = [
  ...
  // The UI plays a short notification sound when a turn finishes, and ships it
  // as a data: URL. Without this it falls back to default-src and is refused —
  // found in a browser, not in review: nothing throws, the sound is just gone.
  "media-src 'self' data: blob:",
  "object-src 'none'",
  "base-uri 'none'",
  "frame-ancestors 'none'",
  // The join page's form is submitted by its own handler, to this origin; the
  // SPA submits none. Without this a form injected into a shell could post the
  // access code the viewer types to an origin of the attacker's choosing,
  // which no other directive here stops (a navigation is not a fetch).
  "form-action 'self'",
]
...
export function createApp(store: Store, bridge?: BridgeClient): Express & { endEventStreams: () => void } {
  ...
  // Ends one departing viewer's SSE streams (see leaveRouter); without the
  // proxy there are none. Bound after the adapter is built, like shareAncestry.
  let endViewerStreams: (viewer_token: string) => void = () => {}
  ...
  // A viewer hands their own token back (see leaveRouter). No body parser: it
  // takes none, and the route is reachable without a token, so nothing an
  // anonymous caller sends is ever buffered. Registered before the proxy so its
  // navigation guard and root mount never see it.
  app.use('/api/leave', exactPathOnly('/api/leave'), leaveRouter(store, (token) => endViewerStreams(token)))
  /**
  ...
  const serveStatic = express.static(PUBLIC_DIR, {
    // No directory index. PUBLIC_DIR/index.html is the UI shell, which the
    // relay serves itself with its CSP and injected scripts (see
    // isStaticPassedOver); as an index it came back raw for '//', and for '/'
    // had the route above not been registered first.
    index: false,
    // Explicit rather than inherited from send: a dotfile in the build (there
    // is none today) is answered as if it were not there, never with the 403
    // that would confirm it is.
    dotfiles: 'ignore',
    // No 301 from /assets to /assets/. With no index to follow it the redirect
    // led to a 404 anyway, and it confirmed which directories exist.
    redirect: false,
    setHeaders(res, filePath) {
      if (isHashedAsset(filePath)) res.setHeader('Cache-Control', 'public, max-age=31536000, immutable')
    },
  })
  // Source maps, HTML documents and _headers are passed over and end at the
  // JSON 404, or at the route that serves the document (see isStaticPassedOver).
  app.use((req, res, next) => (isStaticPassedOver(req.path) ? next() : serveStatic(req, res, next)))
  ...
  if (bridge) {
    ...
    endViewerStreams = adapter.endViewerStreams
...
export async function startServer(port: number = config.port): Promise<http.Server> {
  ...
  // How long a request has to ARRIVE in full, decided here rather than
  // inherited: Node's 300 s default sits within seconds of the largest paste
  // the relay accepts on purpose, and it is also the last bound on how long an
  // admitted body may hold its charge (see httpRequestTimeoutMs, and the
  // inbound watchdog in proxy/adapter.ts). headersTimeout keeps Node's 60 s,
  // which stays under this one, so it is still the header line that is cut
  // first when nothing arrives at all.
