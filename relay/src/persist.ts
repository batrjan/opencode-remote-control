...
export interface PersistedClaim {
  ...
  at: number
  /**
   * The registrant's address block, digested (Store.claimBucketKey), which is
   * what decides whose claim gives way when the map is full. Optional, and
   * absent from a PLAINTEXT file: it is derived from the owner IP, which is
   * stripped there, and a truncated digest of a /24 is only 16.7M guesses away
   * from the address itself. A claim restored without one is treated as a block
   * of its own — the reading that protects it (see Store.restore).
   */
  bucket?: string
}
...
function stripSensitive(state: PersistedState): PersistedState {
  return {
    ...
    // The claim bucket goes with the owner IP: it is a truncated digest of that
    // address's /24, which a plaintext file would put one 16.7M-guess sweep
    // away from the address. Restoring without it costs eviction ordering
    // only — see PersistedClaim.bucket.
    ...(state.claims === undefined
      ? {}
      : {
          claims: state.claims.map((c) => {
            const { bucket: _b, ...safe } = c
            return safe
          }),
        }),
  }
