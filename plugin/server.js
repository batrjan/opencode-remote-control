...
import { clientParentOf, resolveAction, runAction, sanitizeForTerminal, stopShare } from "./bridge-runner.js"
 ...
 *
 * ALIASES COUNT. A word missing here reads as "no subcommand", i.e. as a
 * terminal UI, and the server entry then steps aside in a process that has no
 * TUI entry to take over. opencode 1.18.32 has exactly two aliases, both
 * listed: `auth` for `providers`, and `plug` for `plugin`. The source is
 * `opencode --help`, which prints every command word and its aliases; re-read
 * it when the upstream pin moves (see bridge/test/plugin-subcommands.test.ts,
 * which holds the same list).
 */
const NON_TUI_SUBCOMMANDS = new Set([
  "serve", "run", "web", "attach", "acp", "mcp", "github", "pr", "session", "export", "import",
  "models", "stats", "db", "upgrade", "uninstall", "providers", "auth", "agent", "plugin", "plug",
  "completion", "debug",
])
...
export function showInRunTerminal(text) {
  ...
  // failed stop.
  //
  // Sanitized on the way out: this text carries relay fields, and the terminal
  // would obey the control sequences in them (see sanitizeForTerminal).
  writeAllSync(2, Buffer.from(`${sanitizeForTerminal(text)}\n`))
}
