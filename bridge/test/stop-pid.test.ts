...
/**
 * A stand-in pid: any number but this process's own.
 *
 * The numbers below are opaque — every test that uses one injects its own
 * lookup, so nothing here ever goes near a real process — with one exception.
 * terminateBridgeProcess and terminateSpawnedServer both refuse to signal the
 * CALLER (the library path runs stop inside the bridge process), and they
 * refuse it SILENTLY: no kill, and no warning either. A run whose own pid
 * happens to be the number under test therefore sees the function do nothing,
 * for a reason that has nothing to do with what the test is about.
 *
 * Not hypothetical. On GitHub Actions (run 35310289094) the stop of pid 4246
 * made no call and printed no refusal, in a job log where every other refusal
 * printed one — and the bystanders spawned seven seconds earlier held pids 3881
 * and 3890, so 4246 was squarely in the range that runner was handing out. The
 * same job passed on re-run. Step off our own pid and the number is only ever a
 * stand-in.
 */
function otherPid(pid: number): number {
  return pid === process.pid ? pid + 1 : pid
}

const PID_STAND_IN = otherPid(4242)
const PID_STARTED_BEFORE = otherPid(4243)
const PID_LOOKUP_THROWS = otherPid(4244)
const PID_ALREADY_GONE = otherPid(4245)
const PID_NO_START_TIME = otherPid(4246)
...
test('accepts every shape of bridge command line the launchers produce', () => {
  ...
  for (const command of commands) {
    kill.mockClear()
    terminateBridgeProcess(PID_STAND_IN, undefined, () => ({ command }))
    expect(kill, command).toHaveBeenCalledWith(PID_STAND_IN, 'SIGTERM')
  }
...
test('refuses a pid whose process started after the share was registered', async () => {
  ...
  const kill = vi.spyOn(process, 'kill').mockImplementation(() => true)
  terminateBridgeProcess(PID_STARTED_BEFORE, startedAt, () => ({ command: BRIDGE_COMMAND, startedAt: startedAt - 3_000 }))
  expect(kill).toHaveBeenCalledWith(PID_STARTED_BEFORE, 'SIGTERM')
})
...
test('never throws and never signals the caller', async () => {
  ...
  expect(() =>
    terminateBridgeProcess(PID_LOOKUP_THROWS, Date.now(), () => {
      throw new Error('ps unavailable')
    }),
  ).not.toThrow()
  // A pid nothing is running any more.
  expect(() => terminateBridgeProcess(PID_ALREADY_GONE, Date.now(), () => null)).not.toThrow()
...
test('a snapshot without a start time is judged on the command line alone', () => {
  ...
  terminateBridgeProcess(PID_NO_START_TIME, Date.now(), () => snapshot)
  expect(kill).toHaveBeenCalledWith(PID_NO_START_TIME, 'SIGTERM')
})
...
test('stop terminates the opencode server the bridge spawned', () => {
  ...
  process.kill = spy
  try {
    terminateSpawnedServer(PID_STAND_IN, Date.now(), () => ({
  ...
  expect(signalled).toEqual([PID_STAND_IN])
})
...
test('stop does not terminate a pid that is no longer an opencode server', () => {
  ...
  try {
    for (const command of [
      ...
      terminateSpawnedServer(PID_STAND_IN, Date.now(), () => ({ command, startedAt: Date.now() - 60_000 }))
    }
    // A pid that is simply gone, and our own pid.
    terminateSpawnedServer(PID_STAND_IN, Date.now(), () => null)
...
test('stop does not terminate a server that started after the share was registered', () => {
  ...
  try {
    ...
    terminateSpawnedServer(PID_STAND_IN, shareRegisteredAt, () => ({
