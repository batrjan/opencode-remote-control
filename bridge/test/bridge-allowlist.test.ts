...
/**
 * Always with the bound session: a frame only ever arrives after connect() has
 * set it, and ':id' is now bound to it (see bridge-session-binding.test.ts), so
 * "would this be allowed for the share that registered SES" is the only
 * question the allowlist is ever asked in production.
 */
test.each(ACCEPTED)('accepts %s %s', (method, path) => {
  expect(isProxyRequestAllowed(method, path, SES)).toBe(true)
})
...
const REJECTED: Array<[string, string]> = [
  ...
  // The terminal family, all of it. 'GET /pty' lists the owner's terminals with
  // their command, args, cwd and pid — the owner's command line on a poll, the
  // same disclosure the event filter drops `pty.created` for — and the rest was
  // never routed, so the panel could open nothing anyway.
  ['GET', '/pty?directory=%2Fp'],
  ['GET', '/pty/shells?directory=%2Fp'],
  ['POST', '/pty?directory=%2Fp'],
  ['GET', '/pty/pty_1/connect?directory=%2Fp'],
  ['POST', '/pty/pty_1/connect-token?directory=%2Fp'],
  ['GET', '/api/pty?directory=%2Fp'],
...
test.each(REJECTED)('rejects %s %s', (method, path) => {
  expect(isProxyRequestAllowed(method, path, SES)).toBe(false)
})
...
test('a subagent session id is accepted on child-readable reads', () => {
  ...
  // routes (see readableSessionId); those ids are real ses_… ids. Only their
  // SHAPE passes here — the descent itself is proven against opencode before
  // the request is forwarded (see bridge-session-binding.test.ts).
  ...
  // …and not on the routes that run something.
  expect(isProxyRequestAllowed('POST', '/session/ses_child1/shell', SES)).toBe(false)
})
...
test('the socket handler answers a disallowed request with 403 and never calls opencode', async () => {
  ...
  // A connected bridge always has one, and ':id' is bound to it.
