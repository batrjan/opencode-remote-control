...
test('a start that ended a dead earlier share leaves the state a concurrent start wrote meanwhile', async () => {
  ...
  // A share whose bridge died without a word, its state still on disk — and
  // one a start ENDS rather than takes back, which is what puts a DELETE in
  // the interleaving this test is about. A start only ends a dead share it
  // cannot re-register as itself; this one was recorded against another relay
  // address (the same listener under another origin, so the DELETE still
  // arrives), so its code is not this relay's to present. A share that IS
  // resumable is left alone on the relay — bridge/test/share-resume.test.ts.
  saveSessionState({
    session_id: id,
    access_code: 'DEADCODE',
    bridge_token: 'token-of-a-bridge-that-died',
    relay: relayUrl.replace('127.0.0.1', 'localhost'),
