...
test("a subagent's and a nested subagent's permission prompts are answered", async () => {
  ...
  // The chain is read in the instance the answer goes to, and each link only
  // once: a session already proven to descend from the share is remembered, so
  // the nested prompt's walk stops at its parent instead of re-reading it.
  expect(g.lookups).toEqual([`ses_child${QUERY}`, `ses_grand${QUERY}`])
})
